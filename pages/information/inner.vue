    ...
    <div class="info-inner-container">
      <div class="info-details">
        ...
        <div class="info-content">
          ...
          <p class="time">
            时间:{{
              formatTo(innerData.InnerDetail._addtime)
            }}&nbsp&nbsp阅读量：{{
              innerData.InnerDetail._clickcount
            }}&nbsp&nbsp来源:{{ innerData.InnerDetail._source }}
          </p>
          <p class="gr">导语：{{ innerData.InnerDetail._summary }}</p>
          <p class="paper" v-if="innerData.InnerDetail._contents != undefined">
            <span
              class="paper-content"
              v-html="innerData.InnerDetail._contents.replace(/https/g, 'http')"
              >{{
                innerData.InnerDetail._contents.replace(/https/g, "http")
              }}</span
            >
          </p>
          <div
            class="meting-lf lf"
            v-if="innerData.InnerDetail._specialguest != undefined"
          >
            ...
            <div class="information-box">
              <ul>
                ...
                <li>
                  ...
                  <span
                    >活动时间：{{
                      formatToDate(innerData.InnerDetail._datetime)
                    }}</span
                  >
                </li>
                <li>
                  ...
                  <span
                    >活动类型：{{
                      formatActivityType(innerData.InnerDetail._various)
                    }}</span
                  >
                </li>
                <li>
                ...
                <li>
                  ...
                  <span
                    >参会人数：{{
                      innerData.InnerDetail._participantsnum == null
                        ? 0
                        : innerData.InnerDetail._participantsnum
                    }}
                    人</span
                  >
                </li>
      ...
      <div class="relative-info">
        ...
        <ul class="hotPaperList">
          <li
            v-for="(item, index) in innerData.HotList"
            :key="item._id"
            class="hotPaperItem"
            v-if="index < 3"
          >
...
export default {
  ...
  mounted() {
    if (this.innerData.InnerDetail._contents != undefined) {
      this.innerData.InnerDetail._newcontents = this.innerData.InnerDetail._contents.replace(
        /https/g,
        "http"
      );
    }
